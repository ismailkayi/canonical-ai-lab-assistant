...
initialize_tofu() {
    ...
    if [[ -d "$terraform_dir" ]]; then
        ...
        (cd "$terraform_dir" && tofu init -input=false >/dev/null 2>&1)
        add_done "OpenTofu working directory initialized"
    else
        add_skipped "terraform/ directory not found — skipping tofu init"
    fi
}
...
main() {
    ...
    initialize_tofu
    print_summary
    log_success "Host preparation completed. You can now run: lab-ai chat"
}
