...
if ! command -v tofu >/dev/null 2>&1; then
    log_error "OpenTofu (tofu) not found"
    exit 1
fi
...
if [[ -z "${LXD_PROJECT}" ]]; then
    log_error "Workspace '${WORKSPACE}' has no dedicated LXD project"
    exit 1
fi
...
if ! lxc --project "${LXD_PROJECT}" info "${INITIATOR_NODE}" &>/dev/null; then
...
run_on_node() {
    local node="$1"; shift
    lxc --project "${LXD_PROJECT}" exec "${node}" -- bash -c "$*" \
...
if [[ "${NETWORK_MODE}" == "fully-segregated-4nic" ]]; then
    ...
    for i in $(seq 1 "${NODE_COUNT}"); do
        ...
        if ! lxc --project "${LXD_PROJECT}" info "${node}" >/dev/null 2>&1; then
        ...
        for iface in mgmt0 ovn-uplink ovn-underlay ceph-general; do
            if ! lxc --project "${LXD_PROJECT}" exec "${node}" -- \
        ...
        if lxc --project "${LXD_PROJECT}" exec "${node}" -- \
                ip -o address show dev ovn-uplink 2>/dev/null \
                | grep -Eq 'inet6? '; then
        ...
        actual_ovn_ip=$(lxc --project "${LXD_PROJECT}" exec "${node}" -- sh -c \
            "ip -4 -o address show dev ovn-underlay scope global | awk '{split(\$4,a,\"/\"); print a[1]; exit}'" \
            2>/dev/null || true)
        actual_ceph_ip=$(lxc --project "${LXD_PROJECT}" exec "${node}" -- sh -c \
        ...
        if ! lxc --project "${LXD_PROJECT}" exec "${node}" -- \
                ping -I ovn-underlay -c 1 -W 2 "${peer_ovn_ip}" \
                >/dev/null 2>&1; then
            log_warn "${node}: cannot reach ${peer_ovn_ip} over ovn-underlay"
            OVERALL_OK=false
        fi
        if ! lxc --project "${LXD_PROJECT}" exec "${node}" -- \
                ping -I ceph-general -c 1 -W 2 "${peer_ceph_ip}" \
                >/dev/null 2>&1; then
